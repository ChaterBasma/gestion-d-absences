            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('modules.update', $module) }}">
                        ...
                        <div class="mb-4">
                            <label for="CodeM" class="block text-sm font-medium mb-2">Code (lecture seule)</label>
                            <input type="text" id="CodeM" value="{{ $module->CodeM }}"
                        ...
                        <div class="mb-4">
                            ...
                            <select id="CodeF" name="CodeF"
                                ...
                                @foreach($filieres as $filiere)
                                    <option value="{{ $filiere->CodeF }}" {{ old('CodeF', $module->CodeF) == $filiere->CodeF ? 'selected' : '' }}>
                                        {{ $filiere->Libelle }}
                                    </option>
                                @endforeach
                            </select>
                            @error('CodeF')<span class="text-red-600 text-sm">{{ $message }}</span>@enderror
                        </div>

                        <div class="grid grid-cols-3 gap-4 mb-4">
                            <div>
                                <label for="MHP" class="block text-sm font-medium mb-2">MHP (heures) *</label>
                                <input type="number" id="MHP" name="MHP" value="{{ old('MHP', $module->MHP) }}" min="0" step="0.5"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                                    oninput="calculerMHG()"
                                    required>
                            ...
                            <div>
                                ...
                                <input type="number" id="MHD" name="MHD" value="{{ old('MHD', $module->MHD) }}" min="0" step="0.5"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                                    oninput="calculerMHG()"
                                    required>
                            ...
                            <div>
                                ...
                                <input type="number" id="MHG" name="MHG" value="{{ old('MHG', $module->MHP + $module->MHD) }}" step="0.5" readonly
                        ...
                        <div class="mb-4">
                            ...
                            <select id="coef" name="coef"
                                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                                required>
                                <option value="1" {{ old('coef', $module->coef) == '1' ? 'selected' : '' }}>1</option>
                                <option value="2" {{ old('coef', $module->coef) == '2' ? 'selected' : '' }}>2</option>
                                <option value="3" {{ old('coef', $module->coef) == '3' ? 'selected' : '' }}>3</option>
                            </select>
                            @error('coef')<span class="text-red-600 text-sm">{{ $message }}</span>@enderror
                        </div>
