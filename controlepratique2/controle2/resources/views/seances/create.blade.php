<x-app-layout>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('seances.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="NumS" name="NumS" value="{{ old('NumS') }}"
                            ...
                            <select id="Matricule" name="Matricule"
                                ...
                                @foreach($formateurs as $formateur)
                                    <option value="{{ $formateur->Matricule }}"
                                        {{ old('Matricule') == $formateur->Matricule ? 'selected' : '' }}>
                            ...
                            <select id="CodeG" name="CodeG"
                                ...
                                @foreach($groupes as $groupe)
                                    <option value="{{ $groupe->CodeG }}"
                                        {{ old('CodeG') == $groupe->CodeG ? 'selected' : '' }}>
                            ...
                            <select id="CodeM" name="CodeM"
                                ...
                                @foreach($modules as $module)
                                    <option value="{{ $module->CodeM }}"
                                        {{ old('CodeM') == $module->CodeM ? 'selected' : '' }}>
                                        {{ $module->CodeM }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center">
                                    <input type="radio" name="TypeCours" value="P" {{ old('TypeCours') == 'P' ? 'checked' : '' }} required>
                                    <span class="ml-2">Présentielle</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="radio" name="TypeCours" value="D" {{ old('TypeCours') == 'D' ? 'checked' : '' }} required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="Jour" name="Jour" value="{{ old('Jour') }}"
                                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="time" id="HeureD" name="HeureD" value="{{ old('HeureD') }}"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                                    required>
                                @error('HeureD')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                            ...
                            <div>
                                ...
                                <input type="time" id="HeureF" name="HeureF" value="{{ old('HeureF') }}"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700"
                                    required>
                                @error('HeureF')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="DureeHeures" name="DureeHeures" value="{{ old('DureeHeures') }}" step="0.25"
                            ...
                            <input type="hidden" id="Duree" name="Duree" value="{{ old('Duree') }}">
                            @error('Duree')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="EffAbsent" name="EffAbsent" value="{{ old('EffAbsent', 0) }}" min="0"
                                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700">
                            @error('EffAbsent')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600">
                            ...
                            <div id="stagiaires-container" class="space-y-2 max-h-64 overflow-y-auto">
                                <p class="text-gray-500 dark:text-gray-400 text-sm italic">
                                    Veuillez d'abord sélectionner un groupe pour voir la liste des stagiaires.
                                </p>
                            </div>
    ...
    <script>
        ...
        async function chargerStagiaires() {
            ...
            try {
                ...
                stagiaires.forEach(stagiaire => {
                    const isChecked = (Array.isArray(window.oldAbsences) && window.oldAbsences.includes(stagiaire.CodeS)) ? 'checked' : '';
                    html += `
        ...
        // Charger les stagiaires si un groupe est déjà sélectionné (après erreur de validation)
